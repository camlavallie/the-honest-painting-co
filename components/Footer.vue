<template>
<div class="text-center mt-3">
 <p class="text-center text-gray-500 text-xs mt-12">
            &copy;{{new Date().getFullYear()}} The Honest Painting CO. All rights reserved.
 </p>
 <div class="flex items-center pt-5 justify-between lg:justify-center">
      <a href="https://www.instagram.com/thehonestpaintingco/" target="blank" class="px-2 opacity-50 hover:opacity-100 focus:opacity-100"><img class="w-full" src="../assets/images/insta.svg" alt="" style="max-height: 60px;"></a>

      <p class="px-2 lg:text-xl md:text-xs text-xs opacity-50">Follow us</p>

      <a href="https://www.facebook.com/The-Honest-Painting-Co-888190547983159/" target="blank" class="px-2 opacity-50 hover:opacity-100 focus:opacity-100"><img class="w-full" src="../assets/images/facebook2.svg" alt="" style="max-height: 60px;"></a>
    </div>
</div>
</template>